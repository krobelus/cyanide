...
static const int AUDIO_FRAME_DURATION = 20;
static const int AUDIO_SAMPLE_RATE = 48000;
static const int AUDIO_BIT_RATE = 32; // TODO dew the math
static const int VIDEO_BIT_RATE = 3000;
...
bool Cyanide::answer() {
  ...
  bool success = toxav_answer(toxav, call_friend_number, audio_bit_rate,
...
bool Cyanide::call_control(int fid, int control) {
  ...
  bool success =
      toxav_call_control(toxav, fid, (TOXAV_CALL_CONTROL)control, &error);
  if (!success) {
...
QStringList Call_State::display(int s) {
  return QStringList() << (s & Error ? "Error" : "")
                       << (s & Finished ? "Finished" : "")
...
void Cyanide::_callback_call_state(int fid, int state) {
  ...
  qDebug() << "call with friend" << fid << "changes state from"
           << Call_State::display(call_state) << "to"
           << Call_State::display(state);
...
void Cyanide::audio_thread() {
  ...
  while (in_call) {
    alcGetIntegerv(input_device, ALC_CAPTURE_SAMPLES, sizeof(samples),
                   &samples);
    if (samples >= samples_per_frame) {
      alcCaptureSamples(input_device, buffer, samples_per_frame);
      toxav_audio_send_frame(toxav, call_friend_number, buffer,
                             samples_per_frame, 1, AUDIO_SAMPLE_RATE, &error);
